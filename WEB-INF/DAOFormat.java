...
public class DAOFormat extends GenericServlet {
	...
	private static final String className = "DAOFormat\n";
	...
	public void service(ServletRequest request, ServletResponse response) {
		...
		try {
			...
			try {
				ps = c.prepareStatement("SELECT * FROM T_テーブル名 WHERE 列名=?");
				ps.setString(1, input);
				rs = ps.executeQuery();
				while(rs.next()) {
					output = rs.getString("列名");
				}

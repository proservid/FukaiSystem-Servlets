				...
				try {
					if(param.size() > 1) {
						...
						if(param.size() == 3) {//小
							...
							ps = c.prepareStatement("select 中分類CD,中分類名,sum(金額) as 金額 " +
									" from T_製作_親 pp" +
									" left outer join T_在庫_親 sp on pp.製作期=sp.注文期 and pp.製作番号=sp.注文番号 and pp.製作枝番=sp.注文枝番" +
									" left outer join T_在庫_子 sc on sp.在庫親ID=sc.在庫親ID" +
									" left outer join T_指定納品書 d on sc.納品書番号=d.ID" +
						...
						ps = c.prepareStatement("select 大分類CD,case when 大分類名 is null then '(未分類)' else 大分類名 end as 大分類名,sum(金額) as 金額" +
							" from T_製作_親 pp" +
							" left outer join T_在庫_親 sp on pp.製作期=sp.注文期 and pp.製作番号=sp.注文番号 and pp.製作枝番=sp.注文枝番" +
							" left outer join T_在庫_子 sc on sp.在庫親ID=sc.在庫親ID" +
							" left outer join T_指定納品書 d on sc.納品書番号=d.ID" +

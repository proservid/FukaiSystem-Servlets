...
public class GetShelf extends GenericServlet {
	...
	//クライアントから読み込み
			...
			in.close();
//設計ミス。
//mapでキーとvalue(dto)をひもづけておき、列ごとにselect結果をdtoに格納すべきだった。。。
String sql =
"select distinct" + "\n" +
...
int n = 1;
				...
				while(rs.next()) {
					...
					row.add(rs.getInt("繰越金額")+rs.getInt("繰越90"));
					row.add(rs.getInt("仕入金額"));
					row.add(rs.getInt("出庫金額")+rs.getInt("出庫90"));
					row.add(rs.getInt("仕入金額")+rs.getInt("出庫金額")+rs.getInt("出庫90"));
					row.add(rs.getInt("繰越金額")+rs.getInt("繰越90")+rs.getInt("仕入金額")+rs.getInt("出庫金額")+rs.getInt("出庫90"));

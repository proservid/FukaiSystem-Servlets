...
public class ReadAll extends ServiceFoundation {
	...
	public Object access(Connection c, ServletResponse response, Object o) throws IOException, SQLException {
		...
		if (dto.getMiddle() == 0) { // 年間
			...
			from = LocalDate.of(dto.getCoarse(), dto.getMiddle(), dto.getFine());
			to = from.plusDays(1);
			group = "年月日";
			having = "? IS NOT NULL AND ? IS NOT NULL";
		}
		...
		try (
			PreparedStatement ps = c.prepareStatement(
				...
				// 所定内は切り捨てを行わない実時間のため、保持している値をそのまま合計する
				+ " SUM(所定内) AS 所定内合計,"
				...
				+ " GROUP BY " + group + ", 人員CD HAVING " + having + ") a"
			);
		) {
			int i = 1;
			ps.setObject(i++, from);

 ...
 * 実際原価を取得する
 */
public class GetActualCost extends ServiceFoundation {
		...
		if (params.size() > 1) { // 中分類コードが指定されている
			...
			if (params.size() == 3) { // 小分類コードも指定されている＝小分類を取得
				...
				if (coarseCD > 100) { // コード 101 以上は加工費
					query = "select wc.CD as 小分類CD,小分類名,姓+' '+名 as 仕入先名,substring(convert(varchar,着手日時,20),1,16)+'～'+substring(convert(varchar,終了日時,108),1,5) as 名,"
				...
				String query;
				if (coarseCD > 100) { // コード 101 以上は加工費
					query = "select 中分類CD,中分類名,sum(w.時間*w.単価/100) as 金額 "
						+ " from T_製作_親 pp"
						+ " left outer join T_加工実績 w on pp.製作期=w.製作期 and pp.製作番号=w.製作番号 and pp.製作枝番=w.製作枝番"
						+ " left outer join M_加工_子 wc on wc.CD=w.加工CD"
						+ " left outer join M_加工_親 wp on wp.大分類CD=wc.大分類CD and wp.CD=wc.中分類CD"
					...
					query = "select 中分類CD,中分類名,sum(金額) as 金額  from ("

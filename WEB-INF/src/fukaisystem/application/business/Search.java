...
/**
 * メイン（見積･製作･納品）タブの見積、製作、売上データを検索する
 */
public class Search extends ServiceFoundation {

	@Override
	public Object access(Connection c, ServletResponse response, Object o) throws IOException, SQLException {
		ProjectSearchDTO searchDTO = cast(response, o, ProjectSearchDTO.class);
		...
		String[] constStrs = { "(e.案件名 LIKE ? OR p.案件名 LIKE ?)",
			...
			"SUBSTRING(CONVERT(VARCHAR, 見積年月日),1,4) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 見積年月日),6,2) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 見積年月日),9,2) like ?",
			"製作枝番=?", "受注番号=?",
			"SUBSTRING(CONVERT(VARCHAR, 受注年月日),1,4) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 受注年月日),6,2) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 受注年月日),9,2) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 発行年月日),1,4) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 発行年月日),6,2) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 発行年月日),9,2) like ?",
			"SUBSTRING(CONVERT(VARCHAR, p.納期),1,4) like ?",
			"SUBSTRING(CONVERT(VARCHAR, p.納期),6,2) like ?",
			"SUBSTRING(CONVERT(VARCHAR, p.納期),9,2) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 出荷年月日),1,4) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 出荷年月日),6,2) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 出荷年月日),9,2) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 検収年月日),1,4) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 検収年月日),6,2) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 検収年月日),9,2) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 売上年月日),1,4) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 売上年月日),6,2) like ?",
			"SUBSTRING(CONVERT(VARCHAR, 売上年月日),9,2) like ?",
		};
		String[] constInts = {
			"製作期=?",
			"製作番号=?",
			...
			"見積期=?",
			"見積番号=?",
			"((e.購入者CD=? AND e.種類=?) OR (p.購入者CD=? AND p.種類=?))",
			"(e.通貨CD=? OR p.通貨CD=?)",
			"(見積金額=? OR 契約金額=?)",
			"(e.誕生期=? OR p.誕生期=?)",
			"(e.誕生番号=? OR p.誕生番号=?)" };
		...
		for (int i = 2; i < 28; i++) {
			if (!searchDTO.getStr(i).equals("")) { // 検索条件が入っていれば
				...
				if (searchDTO.getStr(i).equals("未")) {
					switch (i) {
						case 5:
							condition.append("(提出年月日 IS NULL AND 見積年月日 IS NOT NULL)");
							break;
						case 10:
							condition.append("(受注年月日 IS NULL AND (提出年月日 IS NOT NULL OR 発行年月日 IS NOT NULL))");
							break;
						case 13:
							condition.append("(発行年月日 IS NULL AND 提出年月日 IS NOT NULL)");
							break;
						case 16:
							condition.append("(納期 IS NULL AND 発行年月日 IS NOT NULL)");
							break;
						case 19:
							condition.append("(出荷年月日 IS NULL AND 発行年月日 IS NOT NULL)");
							break;
						case 22:
							condition.append("(検収年月日 IS NULL AND 発行年月日 IS NOT NULL)");
							break;
						case 25:
							condition.append("(売上年月日 IS NULL AND 売上年月日 IS NOT NULL)");
							break;
					}
				} else {
		...
		*/
			...
			case 0:// 指定しない
				query.append(
					" GROUP BY mp.製作親ID) sub" + "\n"
						...
						+ ((isFull || !searchDTO.isAnd()) ? " FULL" : "LEFT")
				);
				break;
			case 1:// 未
				query.append(
		...
		if (!conditionStr.equals(""))
			query.append(" WHERE " + conditionStr);
			...
			try (ResultSet rs = ps.executeQuery();) {
				while (rs.next()) {
					...
					if (estNum != 0) {
						if (estNum < 10) {
							record
								.add(rs.getString("見積期") + "-00" + rs.getString("見積番号") + " " + rs.getString("見積枝番"));
						} else if (estNum < 100) {
							record
								.add(rs.getString("見積期") + "-0" + rs.getString("見積番号") + " " + rs.getString("見積枝番"));
						} else {
							record
								.add(rs.getString("見積期") + "-" + rs.getString("見積番号") + " " + rs.getString("見積枝番"));
						}
					} else {
						record.add("");
					}

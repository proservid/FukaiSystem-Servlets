...
import fukaisystem.dto.business.ProjectSummaryDTO;
import fukaisystem.foundation.ServiceFoundation;

/**
 * メイン（見積･製作･納品）タブの情報パネルに表示する見積、製作、売上データを取得する
 */
public class GetSummary extends ServiceFoundation {
	...
	public Object access(Connection c, ServletResponse response, Object o) throws IOException, SQLException {
		...
		IDDTO dto = cast(response, o, IDDTO.class);

		quotationID = dto.getQuotationID();
		productionID = dto.getProductionID();
		salesID = dto.getSalesID();
		// TODO:やや遅い
		...
		try (PreparedStatement ps = c.prepareStatement("SELECT * FROM T_見積_子 WHERE 見積親ID=?");) {
		...
		// 製作明細 TODO:おそい
		try (PreparedStatement ps = c.prepareStatement("SELECT * FROM T_製作_子 WHERE 製作親ID=?");) {

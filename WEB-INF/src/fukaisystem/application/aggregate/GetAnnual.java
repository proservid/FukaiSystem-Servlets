 ...
 *
 */
public class GetAnnual extends ServiceFoundation {
	...
	private static final int PURCHASE_TAX = 5;
	private static final int SALES_1 = 6;
	private static final int SALES_2 = 7;
	private static final int SALES_TAX = 10;
	private static final int QUOTATION = 12;
	private static final int CHECK_DELIVERY = 13;
	private static final int CREATE_DELIVERY = 14;
	private static final int CHECK_ACCOUNT = 15;
	private static final int MH = 16;
	private static final int SALES_MANAGEMENT = 17;
	...
	public Object access(Connection c, ServletResponse response, Object o) throws IOException, SQLException {
		...
		String sql = getQuery(condition.getOrder(), condition.getYear());
		if (sql.isEmpty()) {
			reportException(response, new Exception("年次集計では対応していない集計種別です"));
			return null;
		}
	 ...
	 */
	private String getQuery(int order, int y) {

		String o_period = "";// 注文書の集計期間
		o_period = y + "/1/1' and 納品書日<'" + (y + 1) + "/1/1'";
		...
		switch (order) {
			...
			case SALES_1:// 売①
				query = "select case when 得意先名 is null then '合計' else 得意先名 end as 得意先名," +
						...
						"     where 納品区分CD<5 and 売上年月日>='" + y + "/1/1' and 売上年月日<'" + (y + 1) + "/1/1' and 売上FLG='true') s" +
						...
						"  where 製作番号<9000" +
						"  and not exists(select 1 from V_売上集計ヘッダ WHERE 売上年月日>='" + y + "/1/1' AND 売上年月日<'" + (y + 1) + "/1/1'" +
						...
						"         WHEN 種別CD=4 THEN 会社名+'㈲' + CASE WHEN 支店名 IS NULL THEN '' ELSE ' ' + 支店名 END" +
						"         ELSE 会社名 + CASE WHEN 支店名 IS NULL THEN '' ELSE ' ' + 支店名 END END AS 得意先名," +
						"          SUM(金額) + ROUND(SUM(金額) * (SELECT 税率 FROM M_消費税 t WHERE 適用開始日<'" + (y + 1) + "/1/1'" +
						"          AND NOT EXISTS (" +
						"           SELECT 1 FROM M_消費税 t2 WHERE t.適用開始日<t2.適用開始日 AND 適用開始日<'" + (y + 1) + "/1/1')" +
						"			),0) AS 税込金額" +
						"   FROM V_売上集計ヘッダ h" +
						"   left outer join M_法人 c on h.得意先CD=c.得意先CD" +
						"   WHERE 売上年月日>='" + y + "/1/1' AND 売上年月日<'" + (y + 1) + "/1/1'" +
			...
			case SALES_TAX:// 売消費税
				query = "select convert(integer,SUM(税合計)) as 売上消費税 from (" +
						...
						"  SUM(金額) AS 納入合計," +
						"  round(SUM(金額) * " +
						"   (SELECT 税率 FROM M_消費税 t WHERE 適用開始日<'" + (y + 1) + "/1/1'" +
						"    AND NOT EXISTS (" +
						"     SELECT 1 FROM M_消費税 t2 WHERE t.適用開始日<t2.適用開始日 AND 適用開始日<'" + (y + 1) + "/1/1'" +
						"    )),0) AS 税合計" +
						" FROM V_売上集計ヘッダ" +
			...
			case QUOTATION:// 見積書
				query = "select convert(varchar,見積期)+'-'+right('000' + convert(varchar, 見積番号), 3)+見積枝番 as 見積番号," +
						...
						" where 見積期 >=0 and 見積年月日>='" + y + "/1/1' and 見積年月日<'" + (y + 1) + "/1/1'" +
						" order by 見積番号";
				break;
			case CHECK_DELIVERY:// 納品書入力チェック
				query = "select 納品書番号,合計額,convert(int,round(合計額*税率,0)) as 計算消費税,消費税,消費税-convert(int,round(合計額*税率,0)) as 差額" +
						...
						" left outer join T_在庫_親 p on c.在庫親ID=p.在庫親ID" +
						" left outer join (select 税率 from M_消費税 t where 適用開始日<'" + (y + 1) + "/1/1'" +
						" AND NOT EXISTS (" +
						" SELECT 1 FROM M_消費税 t2 WHERE t.適用開始日<t2.適用開始日 AND 適用開始日<'" + (y + 1) + "/1/1'" +
			...
			case CREATE_DELIVERY:// 注文書から納品書を生成
				query = "select 納品書番号,convert(varchar, 注文期)+'-'+convert(varchar, 注文番号)+convert(varchar, 注文枝番)as 注文番号,p.仕入先CD,会社名," +
						...
						" where 納品書番号>0 and 納品書日>='" + o_period +
						" order by 納品書番号,c.ID";
				break;
			case CHECK_ACCOUNT:// 科目確認
			...
			case MH:// 年次工数サマリー
				query = "select case when 台 is null then '計' else 台+'0' end as 台, convert(float, sum(時間))/100 AS 工数 from " +
						...
						" from T_加工実績 where 着手日時>='" + y + "/1/1'" +
						" and 着手日時<'" + (y + 1) + "/1/1') w group by rollup(台)";
				break;
			case SALES_MANAGEMENT:// 売上管理
				query = "SELECT cast(p.製作期 as varchar)+'-'+cast(p.製作番号 as varchar)+p.製作枝番 AS 製番,会社名,支店名,案件名,売上年月日,売上,CASE WHEN 購入額 IS NULL THEN 0 ELSE 購入額 END+CASE WHEN 出庫額 IS NULL THEN 0 ELSE 出庫額 END+CASE WHEN 加工費 IS NULL THEN 0 ELSE 加工費 END AS 原価,購入額,出庫額,加工費" +
						" FROM (SELECT 製作親ID,MAX(売上年月日) AS 売上年月日,SUM(金額) AS 売上 FROM (SELECT * FROM T_売上_親 WHERE 売上年月日>='" + y + "/1/1' AND 売上年月日<'" + (y + 1) + "/1/1') sp " +

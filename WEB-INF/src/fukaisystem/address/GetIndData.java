			...
			try {
				...
				if(rs.next()) {
					...
					if(rs.getInt("i法人CD") > 0) {
						hasCorpDTO = true;
						corpDTO = new CorpDTO(
								rs.getString("c会社名"), rs.getString("cカイシャメイ"), rs.getString("c支店名"), rs.getString("cシテンメイ"), rs.getString("c表示名"),
								rs.getString("calpha_2"), rs.getString("c郵便番号"), rs.getString("c郵便枝番"), rs.getString("c都道府県"), rs.getString("c市区町村"),
								rs.getString("c町域"), rs.getString("c番地"), rs.getString("c建物等"), rs.getString("cTEL1"), rs.getString("cTEL2"), rs.getString("cTEL3"),
								rs.getString("cFAX1"), rs.getString("cFAX2"), rs.getString("cFAX3"), rs.getString("cメール"), rs.getString("cURL"), rs.getString("c備考"),
								rs.getString("i法人CD"), rs.getString("cアルファベット"), rs.getInt("c仕入先CD"), rs.getInt("c得意先CD"), rs.getInt("c種別CD"), rs.getInt("c年賀状CD"),
								rs.getBoolean("c有効FLG"), rs.getBoolean("c贈答FLG")
								);
					}
					output = new IndDTO(
						rs.getString("i氏名"), rs.getString("iシメイ"), rs.getString("i部署名"), rs.getString("i役職名"), rs.getString("i法人CD"),
						rs.getString("ialpha_2"), rs.getString("i郵便番号"), rs.getString("i郵便枝番"), rs.getString("i都道府県"), rs.getString("i市区町村"), rs.getString("i町域"), rs.getString("i番地"),
						rs.getString("i建物等"), rs.getString("iTEL1"), rs.getString("iTEL2"), rs.getString("iTEL3"), rs.getString("iFAX1"), rs.getString("iFAX2"), rs.getString("iFAX3"), rs.getString("iメール"), "", rs.getString("i備考"), input, rs.getString("社名"),
						rs.getInt("i敬称"), rs.getInt("i年賀状CD"),
						rs.getBoolean("i有効FLG"), rs.getBoolean("i贈答FLG"), rs.getBoolean("i喪FLG"), rs.getBoolean("i自宅FLG"), rs.getBoolean("i住所FLG"), hasCorpDTO, corpDTO
						);
				}

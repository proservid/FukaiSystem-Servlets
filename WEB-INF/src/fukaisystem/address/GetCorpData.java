	...
	public void service(ServletRequest request, ServletResponse response) {
		...
		try {
			...
			try {
				...
				if(rs.next()) {
					output = new CorpDTO(
						rs.getString("会社名"), rs.getString("カイシャメイ"), rs.getString("支店名"), rs.getString("シテンメイ"), rs.getString("表示名"),
						rs.getString("alpha_2"), rs.getString("郵便番号"), rs.getString("郵便枝番"), rs.getString("都道府県"), rs.getString("市区町村"),
						rs.getString("町域"), rs.getString("番地"), rs.getString("建物等"), rs.getString("TEL1"), rs.getString("TEL2"), rs.getString("TEL3"),
						rs.getString("FAX1"), rs.getString("FAX2"), rs.getString("FAX3"), rs.getString("メール"), rs.getString("URL"), rs.getString("備考"),
						input, rs.getString("アルファベット"), rs.getInt("仕入先CD"), rs.getInt("得意先CD"), rs.getInt("種別CD"), rs.getInt("年賀状CD"),

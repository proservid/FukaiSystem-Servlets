	...
	public void service(ServletRequest request, ServletResponse response) {
		...
		try {
			...
			if(rs.next()) {
				...
				dDTO = new DeliveryDTO(deliveryID, 0, 0, rs.getDate("売上年月日"), rs.getInt("納品区分CD"), rs.getInt("納品手段CD"), tax, discount, type, rs.getString("摘要"), null);
			}

			//売上明細
			ps = c.prepareStatement(
//					"SELECT sc.製作親ID,sc.製作子ID,sc.表示CD,出荷伝票番号,pp.受注年月日,sp.売上年月日,pp.受注番号,sc.品名,pc.各FLG,pc.数量,pc.数量単位CD,pc.単価,pc.金額,sc.備考,pc.完成年月日 FROM T_売上_子 sc" +

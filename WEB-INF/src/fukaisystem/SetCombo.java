	...
	public void service(ServletRequest request, ServletResponse response) {
		...
		try {
			...
			try {
				...
				ps = c.prepareStatement("SELECT 機械番号,案件名,製作期,製作番号,製作枝番 FROM T_製作_親" +
						" WHERE 得意先CD=? AND 機械番号>0 ORDER BY 製作期,製作番号");
				ps.setInt(1, code);
				rs = ps.executeQuery();
				while(rs.next()) {
					models.put(rs.getInt("機械番号"), rs.getString("案件名"));
					numbers.put(rs.getInt("機械番号"), new ProductNumber(rs.getInt("製作期"),rs.getInt("製作番号"),rs.getString("製作枝番")));
				}

				ps = c.prepareStatement("SELECT p.CD,氏名 FROM M_個人 p" +

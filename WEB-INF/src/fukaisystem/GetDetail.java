...
import fukaisystem.util.Logging;
	...
	public class GetDetail extends GenericServlet {
				...
				try {
					if(param.size() > 1) {
						...
						if(param.size() == 3) {//小
							m = param.get(2);
							if(l > 100) {
								ps = c.prepareStatement("select wc.CD as 小分類CD,小分類名,姓+' '+名 as 仕入先名,substring(convert(varchar,着手日時,20),1,16)+'～'+substring(convert(varchar,終了日時,108),1,5) as 名," +
									" convert(money,時間)/100 as 数量,単価,w.時間*wc.単価/100 as 金額 " +
									" from T_製作_親 pp" +
								...
								ps = c.prepareStatement("select 小分類CD,小分類名," +
									" CASE" +
							...
							if(l > 100) {
								ps = c.prepareStatement("select 中分類CD,中分類名,sum(w.時間*wc.単価/100) as 金額 " +
									" from T_製作_親 pp" +
									" left outer join T_加工実績 w on pp.製作期=w.製作期 and pp.製作番号=w.製作番号 and pp.製作枝番=w.製作枝番" +
									" left outer join M_加工_子 wc on wc.CD=w.加工CD" +
									" left outer join M_加工_親 wp on wp.大分類CD=wc.大分類CD and wp.CD=wc.中分類CD" +
								...
								ps = c.prepareStatement("select 中分類CD,中分類名,sum(金額) as 金額  from (" +
						...
						ps = c.prepareStatement("select 大分類CD,case when 大分類名 is null then '(未分類)' else 大分類名 end as 大分類名,sum(金額) as 金額 from (" +
							...
							" group by 大分類CD,大分類名" +
							" union all" +
							" select 大分類CD,大分類名,sum(w.時間*wc.単価/100) as 金額 " +
							" from T_製作_親 pp" +
							" left outer join T_加工実績 w on pp.製作期=w.製作期 and pp.製作番号=w.製作番号 and pp.製作枝番=w.製作枝番" +
							" left outer join M_加工_子 wc on wc.CD=w.加工CD" +

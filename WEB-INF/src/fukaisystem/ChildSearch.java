...
//////////////////////////////////////////////////////////////////
				switch(type) {
					case 1://見積
						query.append("(select 見積親ID from T_見積_子 where " + sb.toString() + " group by 見積親ID) a" +
						...
						break;
					case 2://製作
						query.append("(select 製作親ID from T_製作_子 where " + sb.toString() + " group by 製作親ID) a" +
						...
						break;
					case 3://売上
						query.append("(select 製作親ID,売上親ID from T_売上_子 where " + sb.toString() + " group by 製作親ID,売上親ID) a" +
							" left outer join T_製作_親 p on a.製作親ID=p.製作親ID" +
							" left outer join T_見積_親 e on p.見積親ID=e.見積親ID" +
							" left outer join T_製作_親 p2 on e.元製作親ID=p2.製作親ID" +

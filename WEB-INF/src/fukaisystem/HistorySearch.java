			...
			try {
				...
				if(isStock) {
					...
					query.append(
							 ...
							 //" union all" +
							 //" SELECT sc.在庫親ID,sc.ID,0 as 仕入先CD,'出庫',大分類CD,中分類CD,小分類CD,品名,出庫年月日,null,null," +
							 //" 製作期,製作番号,製作枝番,数量,0 as 残数,数量単位CD,重量長さ,単価,金額,'' as 伝票番号,'' as 納品書番号,sc.備考,用途,摘要" +
							 //" FROM T_出庫_親 sp" +
				...
				for(int i = 0; i < 7; i++) {
					if(historyDTO.getInt(i) != 0) {//検索条件が入っていれば
						...
						if(isFirst) {
							...
							if(historyDTO.isAnd()) query.append(" AND " + constInts[i]);
